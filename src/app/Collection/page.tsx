...
function ProductCollection() {
  ...
  const [productSort, setProductSort] = useState<undefined | string>();
  ...
  const loadInitialData = async () => {
    ...
    const { data, errors } = await storeFront(print(productQuery), {
      tag: `tag:${!selectedOption ? "watches" : selectedOption}`,
      cursor: null,
        ...
        <div className="flex-[3] flex flex-col">
          <div className="min-h-[100vh] flex flex-wrap items-center justify-center">
            {productsList?.map((item, i) => (
              ...
              >
                <Card item={item} accessories={false} key={item.node.handle} />
              </motion.div>
            ))}
          </div>
          {moreLoading ? (
